...
def black_options_repo(git_repo_m: GitRepoFixture) -> dict[str, Path]:
    """Fixture for the `test_black_options` test."""
    (git_repo_m.root / "pyproject.toml").write_bytes(b"[tool.black]\n")
    (git_repo_m.root / "black.cfg").write_text(
        dedent(
            """
            [tool.black]
            line-length = 81
...
def test_black_options(black_options_repo, options, expect):
    ...
    black_options_repo["main.py"].write_bytes(b'print ("Hello World!")\n')
    with patch.object(black_diff, "Mode", wraps=black_diff.Mode) as file_mode_class:

        main(options + [str(path) for path in black_options_repo.values()])

    assert black_options_repo["main.py"].read_bytes() == b'print("Hello World!")\n'
